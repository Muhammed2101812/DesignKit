...
export function ToolWrapper({
  ...
  // Get current tool info from pathname
  const currentTool = pathname ? getToolByPath(pathname) : null

  const handleBack = () => {
    router.push('/#tools')
  }
  
  // Info content component for reuse in both dialog and bottom sheet
  const InfoContent = () => (
